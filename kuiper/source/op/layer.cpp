...
namespace op {
    ...
    base::DataType BaseLayer::data_type() const { return data_type_; }//const在后不修改成员变量 放前边限制返回值不能修改
    ...
    base::Status Layer::check_tensor_with_dim(const tensor::Tensor &tensor,
        ...
        for (int32_t i = 0; i < dims; ++i) {
            int32_t dim = va_arg(args, int32_t);//va_arg 从 va_list 结构中 依次读取下一个参数，并且按你指定的类型返回它。
    ...
    void Layer::set_input(int32_t idx, const tensor::Tensor &input) {
        //把某个张量 input 设置到 inputs_ 数组中的第 idx 个位置上。
        CHECK_GE(idx, 0);
