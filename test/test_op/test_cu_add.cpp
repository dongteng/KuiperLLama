...
TEST(test_add_cu, add1_nostream) {
    ...
    int32_t size = 32 * 151;//创建张量的大小 即要处理4832个float值
    ...
    set_value_cu(static_cast<float *>(t1.get_buffer()->ptr()), size, 2.f);//分别将 t1 填充为 2.0，t2 填充为 3.0。
...
TEST(test_add_cu, add1_stream) {
    ...
    kernel::get_add_kernel(base::DeviceType::kDeviceCUDA)(t1, t2, out, stream);//这里不还是传了1个stream?  表现为穿行 支持异步并发（等将来传多个stream）
    cudaDeviceSynchronize();
    float *output = new float[size];
    cudaMemcpy(output, out.ptr<float>(), size * sizeof(float), cudaMemcpyDeviceToHost);
    for (int i = 0; i < size; ++i) {
